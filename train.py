...
def forward(sample_batched, model):
    img, text, gt_gauss, _ = sample_batched
...
train_dataset = LanguageKeypointsDataset(NUM_KEYPOINTS, './data/%s/train'%dataset_dir, IMG_HEIGHT, IMG_WIDTH, transform, gauss_sigma=GAUSS_SIGMA)
train_data = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=workers)

test_dataset = LanguageKeypointsDataset(NUM_KEYPOINTS, './data/%s/test'%dataset_dir, IMG_HEIGHT, IMG_WIDTH, transform, gauss_sigma=GAUSS_SIGMA)
